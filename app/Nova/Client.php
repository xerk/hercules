<?php

namespace App\Nova;

use Laravel\Nova\Fields\ID;
use Illuminate\Http\Request;
use Laravel\Nova\Fields\Text;
use Laravel\Nova\Http\Requests\NovaRequest;

class Client extends Resource
{
    /**
     * The model the resource corresponds to.
     *
     * @var string
     */
    public static $model = \App\Models\Client::class;

    /**
     * The single value that should be used to represent the resource when being displayed.
     *
     * @var string
     */
    public static $title = 'name';

    /**
     * The columns that should be searched.
     *
     * @var array
     */
    public static $search = [
        // 'name',
        // 'unique_id',
        // 'username',
        // 'email'
        'religion'
    ];

    /**
     * Get the fields displayed by the resource.
     *
     * @param  \Illuminate\Http\Request  $request
     * @return array
     */
    public function fields(Request $request)
    {
        return [
            ID::make(__('ID'), 'id')->sortable(),

            Text::make(__('Facebook ID'), 'unique_id')
                ->sortable()
                ->exceptOnForms()
                ->rules('required', 'max:255'),

            Text::make(__('Name'), 'name')
                ->sortable()
                ->exceptOnForms()
                ->rules('required', 'max:255'),

            Text::make(__('Mobile'), 'mobile')
                ->sortable()
                ->exceptOnForms()
                ->rules('required', 'max:255'),

            Text::make(__('Username'), 'username')
                ->sortable()
                ->exceptOnForms()
                ->rules('required', 'max:255'),

            Text::make(__('Email'), 'email')
                ->sortable()
                ->exceptOnForms()
                ->rules('required', 'max:255'),

            Text::make(__('Religion'), 'religion')
                ->sortable()
                // ->onlyOnDetail()
                ->rules('required', 'max:255'),

            Text::make(__('Birthday'), 'birthday')
                ->sortable()
                ->onlyOnDetail()
                ->rules('required', 'max:255'),

            Text::make(__('Gender'), 'gender')
            ->sortable()
            ->exceptOnForms()
            ->rules('required', 'max:255'),
            
            Text::make(__('Work'), 'work')
                ->sortable()
                ->exceptOnForms()
                ->rules('required', 'max:255'),
                
            Text::make(__('Position'), 'position')
                ->sortable()
                ->exceptOnForms()
                ->rules('required', 'max:255'),

            Text::make(__('Hometown'), 'hometown')
                ->sortable()
                ->exceptOnForms()
                ->rules('required', 'max:255'),

            Text::make(__('Location'), 'location')
                ->sortable()
                ->exceptOnForms()
                ->rules('required', 'max:255'),

            Text::make(__('Education'), 'education')
                ->sortable()
                ->exceptOnForms()
                ->rules('required', 'max:255'),

            Text::make(__('Relationship'), 'relationship')
                ->sortable()
                ->onlyOnDetail()
                ->rules('required', 'max:255'),

            Text::make(__('Nationality'), 'nationality')
                ->sortable()
                ->exceptOnForms()
                ->rules('required', 'max:255'),
        ];
    }

    /**
     * Get the cards available for the request.
     *
     * @param  \Illuminate\Http\Request  $request
     * @return array
     */
    public function cards(Request $request)
    {
        return [];
    }

    /**
     * Get the filters available for the resource.
     *
     * @param  \Illuminate\Http\Request  $request
     * @return array
     */
    public function filters(Request $request)
    {
        return [];
    }

    /**
     * Get the lenses available for the resource.
     *
     * @param  \Illuminate\Http\Request  $request
     * @return array
     */
    public function lenses(Request $request)
    {
        return [];
    }

    /**
     * Get the actions available for the resource.
     *
     * @param  \Illuminate\Http\Request  $request
     * @return array
     */
    public function actions(Request $request)
    {
        return [];
    }
}
